        ...
        if (!(rh.flag & REPLAY_SINGLE_MODE))
        {
            if (!(rh.flag & REPLAY_TAG))
            {
                for (int i = 0; i < 2; ++i)
                {
                    for (const auto &code : cur_replay.decks[i].main)
                        new_card(pduel, code, i, i, LOCATION_DECK, 0, POS_FACEDOWN_DEFENSE, 0);
                    for (const auto &code : cur_replay.decks[i].extra)
                        new_card(pduel, code, i, i, LOCATION_EXTRA, 0, POS_FACEDOWN_DEFENSE, 0);
            ...
            else
            {
                for (const auto &code : cur_replay.decks[0].main)
                    new_card(pduel, code, 0, 0, LOCATION_DECK, 0, POS_FACEDOWN_DEFENSE, 0);
                for (const auto &code : cur_replay.decks[0].extra)
                    new_card(pduel, code, 0, 0, LOCATION_EXTRA, 0, POS_FACEDOWN_DEFENSE, 0);
                ...
                for (const auto &code : cur_replay.decks[1].main)
                    new_tag_card(pduel, code, 0, LOCATION_DECK, 0);
                for (const auto &code : cur_replay.decks[1].extra)
                    new_tag_card(pduel, code, 0, LOCATION_EXTRA, 0);
                for (const auto &code : cur_replay.decks[2].main)
                    new_card(pduel, code, 1, 1, LOCATION_DECK, 0, POS_FACEDOWN_DEFENSE, 0);
                for (const auto &code : cur_replay.decks[2].extra)
                    new_card(pduel, code, 1, 1, LOCATION_EXTRA, 0, POS_FACEDOWN_DEFENSE, 0);
                ...
                for (const auto &code : cur_replay.decks[3].main)
                    new_tag_card(pduel, code, 1, LOCATION_DECK, 0);
                for (const auto &code : cur_replay.decks[3].extra)
                    new_tag_card(pduel, code, 1, LOCATION_EXTRA, 0);
            }
        }
        else
        {
