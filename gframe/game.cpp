...
namespace ygo
{
    ...
    void Game::MainServerLoop()
    {
        ...
        {
            std::fprintf(stderr, "Failed to load strings.conf\n");
            return;
        }
        LoadExpansions();
                                       ...
                                       const std::vector<ClientCard *> &cards)
    {
        ...
        if (labelHeight < minTextHeight)
            labelHeight = minTextHeight;
        ...
        if (btnOKHeight < minTextHeight)
            btnOKHeight = minTextHeight;
    ...
    void Game::FixMacOSBundleWorkingDirectory()
    {
        ...
        {
            char path[PATH_MAX];
            if (CFStringCompare(bundle_ext, CFSTR("app"), kCFCompareCaseInsensitive) == kCFCompareEqualTo && CFURLGetFileSystemRepresentation(bundle_base_url, true, (UInt8 *)path, PATH_MAX))
            {
                chdir(path);
            }
